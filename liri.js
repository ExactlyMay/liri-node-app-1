...
function parseCommand(command, arg) {
	...
	case undefined:
	case "":
		console.log("Try again bae, this time i'll listen. I promise.");
		break;

	default:
		console.log("Dude what are you saying? Just try again smh.");
		break;
...
function getTweets() {
	console.log("Once sec, finding tweets.\n");
	var twitter = new Twitter(keys.twitterKeys);
    var params = {screen_name: 'Jordon94304813'};

    twitter.get('statuses/home_timeline', params,function(error, tweets, response) {
    
        if (tweets.length < 1) {
			console.log("Do you even tweet bro? Try again after you've tweeted something.");
			return;
		}
		if (tweets.length === 1) {
...
function spotifySong(song) {
	song = (song || "The Sign");
	console.log("Ouuu I like that song, one sec while I get it for you.\n");
	...
	request(queryUrl, function (error, response, body) {
		...
		if (body && JSON.parse(body).Search && JSON.parse(body).Search.length > 0) {
				...
				if (result.Title.toLowerCase() === movie.toLowerCase()) {
					...
					request(innerQueryURL, function (error, response, body) { 
						...
						if (body && JSON.parse(body) && JSON.parse(body).Response === "True") {
							...
							console.log("I found your movie, it's no Kill Bill but here is some info on it:\n");
			...
			request(innerQueryURL, function (error, response, body) {
				...
				if (body && JSON.parse(body) && JSON.parse(body).Response === "True") {
					body = JSON.parse(body);
					console.log("Yeah so that movie doesn't exist. This was the closest match I could find:\n");
					console.log("  Title: " + body.Title);
					console.log("  Year: " + body.Year);
					for (var j = 0; j < body.Ratings.length; j++) {
						if (body.Ratings[j].Source === "Internet Movie Database") {
